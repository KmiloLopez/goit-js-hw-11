...
searchForm.addEventListener('submit', e => {
  ...
  const URL =
    ...
    API_KEY +
    '&q=' +
    encodeURIComponent(enteredText) +
...
const fetchImages = async theUrl => {
  const clearUrl = `${theUrl}`;
  const foundArray = await axios.get(clearUrl); //busqueda de la url en la API

  const foundArrayJson = await foundArray.data; //acceso a los datos
...
/*  const fetchImages = async(theUrl)=>{
  ...
  console.log("aaaaaaaaaaaa",foundArrayJson)
  return foundArrayJson 
}  */

function printImageCard(arrayImages) {
  //aca llega el array de lo que encontro la busqueda
